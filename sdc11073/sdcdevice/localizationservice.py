...
def _filterByHandles(texts, requestedHandles):
    if requestedHandles is None or len(requestedHandles) == 0:
        ...
        return [t for t in texts if t.Ref in requestedHandles]

def _filterByLanguages(texts, requestedLangs):
    if requestedLangs is None or len(requestedLangs) == 0:
        return texts
    else:
        return [t for t in texts if t.Lang in requestedLangs]

def _filterByVersion(texts, requested_version):
    if requested_version is None:
        # filter requested versions. We need to do it per language and Ref, therefore create a lookup with (ref,lang) as key
        tmp_dict = defaultdict(list)
        for t in texts:
            tmp_dict[(t.Ref, t.Lang)].append(t)
        texts = []
        for texts_list in tmp_dict.values():
            if len(texts_list) >= 2:
                versions = [a.Version for a in texts_list if a.Version is not None]
                if len(versions) >= 2:
                    ...
                    max_version = None
                texts.extend([t for t in texts_list if t.Version == max_version])
            else:
        ...
        return [t for t in texts if t.Version == requested_version]

def _filterByDimension(texts, textWidths, numberOfLines):
    ...
    if len(textWidths) > 0 or len(numberOfLines) > 0:
        ...
        # create dictionary by ref and language:
        tmp_dict = defaultdict(list)
        for t in texts:
            tmp_dict[(t.Ref, t.Lang)].append(t)
        tmp = []
...
class LocalizationStorage:
    ...
    def filterLocalizedTexts(self, requestedHandles, requestedVersion, requestedLangs, textWidths, numberOfLines):
        ...
        texts = _filterByHandles(self._flatList(), requestedHandles)
        texts = _filterByLanguages(texts, requestedLangs)
        texts = _filterByVersion(texts, requestedVersion)
        texts = _filterByDimension(texts, textWidths, numberOfLines)
        return texts
